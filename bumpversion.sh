...
sed -e "s/^__version__ = '${OLD}'$/__version__ = '${NEW}'/" dddp_chat/__init__.py > .__init__.py
mv .__init__.py dddp_chat/__init__.py
#sed -e "s/^version = '${OLD%.*}'/version = '${NEW%.*}'/" -e "s/^release = '${OLD}'/release = '${NEW}'/" docs/conf.py > docs/.conf.py
#mv docs/.conf.py docs/conf.py
git diff setup.py dddp_chat/__init__.py #docs/conf.py
